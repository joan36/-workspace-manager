...
export default function UserDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) setGwUser(await res.json())
    } catch (e) {}
    setLoading(false)
  }, [email])
  ...
  if (!gwUser) return (
    <div style={{ textAlign:"center", padding:"4rem", color:"#6b7280", fontSize:14 }}>
  ...
  return (
    <div style={{ maxWidth:880, margin:"0 auto" }}>
      ...
      <button onClick={() => router.back()} style={{ display:"flex", alignItems:"center", gap:4, fontSize:12, color:"#6b7280", background:"none", border:"none", cursor:"pointer", marginBottom:16, padding:0 }}>
      ...
      {/* Hero */}
      <div style={{ background:"white", border:"0.5px solid #e5e7eb", borderRadius:12, padding:24, marginBottom:14 }}>
        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:20 }}>
          <div style={{ display:"flex", alignItems:"center", gap:18 }}>
            {gwUser.thumbnailPhotoUrl ? (
              <img src={gwUser.thumbnailPhotoUrl} style={{ width:80, height:80, borderRadius:"50%", border:"2px solid #BFDBFE", objectFit:"cover" }} alt="" />
            ) : (
            ...
            <div>
              <div style={{ fontSize:18, fontWeight:500, color:"#111827", marginBottom:3 }}>{gwUser.name?.fullName}</div>
              <div style={{ fontSize:13, color:"#6b7280", marginBottom:6 }}>{gwUser.primaryEmail}</div>
        ...
        <div style={{ display:"flex", gap:28, paddingTop:16, borderTop:"0.5px solid #e5e7eb", flexWrap:"wrap" }}>
          {[
            ...
            <div key={label as string} style={{ display:"flex", flexDirection:"column", gap:2 }}>
              ...
              <span style={{ fontSize:13, fontWeight:500, color:"#111827" }}>{val as string}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display:"flex", borderBottom:"0.5px solid #e5e7eb", marginBottom:14 }}>
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} style={{ padding:"9px 16px", fontSize:13, cursor:"pointer", background:"none", border:"none", borderBottom: activeTab===tab.id ? "2px solid #2563eb" : "2px solid transparent", color: activeTab===tab.id ? "#2563eb" : "#6b7280", fontWeight: activeTab===tab.id ? 500 : 400, marginBottom:-0.5 }}>
            {tab.label}
          </button>
        ))}
      </div>
      ...
      {activeTab === "info" && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
          ...
          <div style={{ background:"white", border:"0.5px solid #e5e7eb", borderRadius:12, padding:"16px 18px" }}>
            ...
            <p style={{ fontSize:12, color:"#9ca3af" }}>Conectar AD para ver datos</p>
          </div>
        </div>
      )}
